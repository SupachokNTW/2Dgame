...
let initialBallSpeed = speedSettings[difficulty];
...
diffButtons.forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        diffButtons.forEach(b => b.classList.remove("selected-diff"));
        btn.classList.add("selected-diff");

        initialBallSpeed = speedSettings[difficulty];

        balls.forEach(ball => {
            let angle = Math.atan2(ball.dy, ball.dx);
            ball.dx = Math.cos(angle) * initialBallSpeed;
            ball.dy = Math.sin(angle) * initialBallSpeed;
        });
    });
});
...
toggleSoundBtn.addEventListener("click", () => {
    volumeOn = !volumeOn;
    toggleSoundBtn.textContent = `Sound: ${volumeOn ? "ON" : "OFF"}`;
});
